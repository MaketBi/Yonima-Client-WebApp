...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title,
    description,
    keywords: [vendor.name, 'livraison', 'Dakar', 'commerce', 'commande en ligne'],
    openGraph: {
      title: vendor.name,
      description,
      type: 'website',
      siteName: APP_NAME,
      url: `${siteUrl}/commerces/${vendor.slug}`,
    ...
    alternates: {
      canonical: `/commerces/${vendor.slug}`,
    },
  };
}
...
async function CommerceContent({ slug }: { slug: string }) {
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      ...
      <ProductModal products={products} vendor={vendor} basePath="/commerces" />
      <PackModal packs={packs} vendor={vendor} basePath="/commerces" />
    </div>
  );
}
